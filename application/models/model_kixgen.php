 ...
 *  Extended by KiXcellent in 2009-10-*
 *
...
class model_kixgen extends CI_Model 
{
...
*/              
        ...
        **/
        ...
        $name_view_show     = 'show_' . $name_table . '.php'; 
        $name_view_list     = 'list_' . $name_table . '.php';
        $name_view_form     = 'form_' . $name_table . '.php'; 
                ...
                if( $this->table_config[$key]['sf_type'] == 'related' )
                {
                    ...
                    **/                                                                                 
                    if( !in_array( $rel_data[0], $field_register ) )
                    {
                        ...
                        if( $rel_data[0] !== $name_table )
                        {
                            $model_joins    .= "\$this->db->join( '$rel_data[0]', '$name_table.$field = $rel_data[0].$rel_data[1]', 'left' );\n";
                        }
                        ...
                        $model_related  .= "\tfunction related_$rel_data[0]()
    {
        ...
        return \$rel_data->result_array();
                 ...
                 *  PLEASE NOTE: CI_KixGen assumes, that a "table1_table2" switch table exists with the ID names of the two table                 
                 */
                        ...
                        $model_related  .= "\tfunction related_$rel_data[0]()
    {
        ...
        **/                         
        if( isset( $data_path['input_config'] ) )
        {
            if( file_exists( $data_path['input_config'] . 'database.tpl' ) )
            {
                $database_config_path = $data_path['output_config'] . 'database.php';

                // if( !file_exists( $database_config_path ) )
                // {

                    $database_config_content = read_file( $data_path['input_config'] . 'database.tpl' );

                    // Replacements
                    $database_config_content = str_replace( "%DATABASE_NAME%", $this->db->database, $database_config_content );

                    file_put_contents( $database_config_path, $database_config_content ); // Create file
              //  }
            }
        }
        ...
        /**
         *   Generate Frame_admin.php VIEW templates
        ...
        **/
        ...
        if( isset( $data_path['input_views'] ) )
        {
            ...
            {
                $frame_view_path = $data_path['output_views'] . 'frame_admin.php';
                ...
                if( !file_exists( $frame_view_path ) )
                {
                    ...
                    {
                        if( $t !== 'sf_config' ) $list_of_tables .= IN6 . "<li class='<?php if(isset(\$table_name)){ if(\$table_name == '".ucfirst($t)."'){ echo 'active'; } } ?>'><a href='$t'><i class='icon-tasks'></i> ".ucfirst($t)."</a></li>\n";
                    }
                    $list_of_tables .= IN5 . '';
            ...
            if( file_exists( $data_path['input_views'] . 'show.tpl' ) )
            {
                ...
                foreach ( $fields as $key => $field )
                {
                    if( $this->table_config[$key]['sf_type'] == 'many_related' )
                    {
                        ...
                        $fields_replacement .= '<tr class="">
                            <td><?php echo $'. $name_table .'_fields[\''. $field .'\']; ?>:</td>
                            <td>
                                <?php foreach($'. $name_table .'_'.$rel_data[0].'_data as $rel) : ?>
                                    <?php echo $rel[\''.$rel_data[3].'\']; ?><br>
                                <?php endforeach; ?>
                            </td>
                        </tr>';
                    }
                    else
                    {
                        $fields_replacement .= '<tr class="">
                            <td><?php echo $'. $name_table .'_fields[\''. $field .'\']; ?>:</td>
                            <td><?php echo $'. $name_table .'_data[\''.$field.'\']; ?></td>
                        </tr>';
                    }
                }
            }
        ...
        {            
            if( file_exists( $data_path['input_views'] . 'list.tpl' ) )
            {
                ...
                foreach ( $fields as $key => $field )
                {
                    ...
                    // If the user clicked 'hidden' in the config, then CI_KixGen doesn't list it
                    if( $hidden_field == 0 )
                    {
                        $table_header .= IN3 . '<th><?php echo $' . $name_table . "_fields['". $field ."']; ?></th>\n";
                        
                        // Test realted table
                        if( $this->table_config[$key]['sf_type'] == 'many_related' )
                        {
                            $table_contents .= IN . "<td>\n";
                            $table_contents .= IN5 . "<?php foreach(\$". $name_table ."_related_fields as \$key=>\$rel) : ?>\n";
                            $table_contents .= IN6 . "<?php if(\$rel['". $name_table ."_id'] == \$row['" . $fields_id . "']){ echo \$rel['" . $rel_data[3] . "']; echo '<br>'; } ?>\n";
                            $table_contents .= IN5 . "<?php endforeach; ?>\n";
                            $table_contents .= IN . "</td>\n";
                        }
                        else
                        {
                            $table_contents .= IN . '<td><?php echo $row[\''. $field ."']; ?></td>\n";
                        }
                    }
                }
